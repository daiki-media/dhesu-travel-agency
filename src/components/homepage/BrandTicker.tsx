    ...
    <div className="flex items-center justify-center mx-8 shrink-0 w-44 h-28 text-primary-dark select-none">
      {brand.type === "script" && (
        <div className="text-center border border-primary/30 rounded px-3 py-2 w-full">
          <p className="text-[7px] tracking-[3px] uppercase opacity-90">{brand.top}</p>
          <div className="flex items-center gap-1 my-0.5 justify-center">
            <span className="text-[7px] tracking-wider opacity-90">✕</span>
            <span className="text-[8px] tracking-[2px] uppercase opacity-90">TRADE MARK</span>
            <span className="text-[7px] tracking-wider opacity-90">✕</span>
          </div>
          <p className="font-dancing text-2xl font-bold leading-none">{brand.main}</p>
          <p className="text-[8px] italic tracking-wide opacity-90 mt-0.5">{brand.sub}</p>
          <p className="text-[6px] tracking-[2px] uppercase opacity-90 mt-1">{brand.bottom}</p>
        </div>
      )}
      {(brand.type === "badge" || brand.type === "explore") && (
        <div className="text-center">
          <p className="text-[7px] tracking-[2px] uppercase opacity-90 mb-1">{brand.top}</p>
          ...
          <p className="text-[7px] tracking-[2px] opacity-90 mt-1">{brand.sub}</p>
        </div>
      )}
      {brand.type === "tall" && (
        <div className="text-center">
          <p className="text-[6px] tracking-[2px] uppercase opacity-90">{brand.top}</p>
          ...
          <p className="text-[6px] tracking-[1px] opacity-90 mt-0.5">{brand.bottom}</p>
        </div>
      )}
      {brand.type === "adventure" && (
        <div className="text-center">
          <p className="text-[7px] tracking-[2px] uppercase opacity-90">{brand.top}</p>
          ...
          <p className="text-[6px] tracking-[2px] opacity-90 mt-1">{brand.bottom}</p>
        </div>
      )}
    </div>
  );
